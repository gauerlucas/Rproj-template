...
# Reminder of available data

## File structure :

In `data/` : original raw data

-   `sub-XXX/` : a folder for every patient
...
library(zoo) # Easy rolling averages computation with rollapplyr
```
...
## Import common data (subject list)
...
# Subject-specific data import and pre-processing script template

## Huge and ugly function to extract individual data from every patients
...
# Run extraction function for every subject in the summary database table.
...
# Export extraction common to all patients
